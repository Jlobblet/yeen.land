...
module Function =
    ...
    let FunctionHandler (event: S3Event) (context: ILambdaContext) =
        ...
        if event.Records = null then
            failwith "Records is null - exiting"
        ...
        let deleteItems, addToDatabaseItems =
            ...
            |> Seq.map (fun r ->
                async {
                    ...
                    let collisions = Reader.run (GetTableContents conditions) services
        ...
        if not <| Seq.isEmpty deleteItems then
            ...
            |> Array.iter (fun (yl, cs) ->
                context.Logger.LogLine
                <| $"Deleting object %s{yl.S3Key} (collisions: %A{cs})"
